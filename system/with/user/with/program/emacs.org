...
Enforces links in top-level '* Required Reading' section. Avoids duplicates in SEEN.
Supports org-roam IDs and absolute file paths (which act as dead ends).
FILE-SEEN is list of file paths already included."
  (interactive)
  (let* ((seen (or seen (list id)))
         (file-seen (or file-seen '()))
         (node (org-roam-node-from-id id))
         (file (and node (org-roam-node-file node)))
         (title (and node (org-roam-node-title node)))
         result required-reading)
    (unless node (cl-return-from my/gptel-org-roam-node-content-by-id nil))
    (with-temp-buffer
      (insert-file-contents file)
      (org-mode)
      ...
      (let ((content (buffer-substring-no-properties
        ...
        ;; Collect Required Reading links/paths in order; account for '- [[...]]' org lists
        (save-excursion
          (goto-char (org-roam-node-point node))
          (when (re-search-forward \"^\\*+ Required Reading$\" nil t)
            (let ((start (point))
              ...
              (let ((section (buffer-substring-no-properties start end)))
                (with-temp-buffer
                  (insert section)
                  (goto-char (point-min))
                  ;; Regex: match lines beginning with a dash, whitespace, then org/link
                  (while (re-search-forward
                          "- +\\\\[\\\\[\\(id:[^]]+\\|/[^]]+\\)\\]\\(?:\\[\\([^]]*\\)\\]\\)?\\\\]" nil t)
        ...
        (dolist (link-pair required-reading)
          (let ((link (car link-pair))
            ...
            (cond
             ;; Org-roam node
             ((and (string-prefix-p \"id:\" link)
                   (not (member link seen)))
              (let ((child-results (my/gptel-org-roam-node-content-by-id link (cons link seen) file-seen)))
                (setq result (append result child-results))) )
             ;; Absolute file path (dead end, only add if not seen)
             ((and (string-prefix-p \"/\" link)
                   (not (member link file-seen))
                   (file-readable-p link))
              (condition-case nil
                  (let ((f-content
                          (with-temp-buffer
...
(defun my/gptel-org-roam-node-content-by-id (id &optional seen file-seen)
  "Return list of plists for ID and its 'Required Reading' tree.
Each plist: (:id ID :title TITLE :content CONTENT :file FILE).
Supports both org-roam node links and absolute file paths."
  (interactive)
  (let* ((seen (or seen (list id)))
         (file-seen (or file-seen '()))
         (node (org-roam-node-from-id id)))
          ...
          (let ((content (buffer-substring-no-properties
            ...
            (dolist (link-pair required-reading)
              (let ((link (car link-pair))
                    (desc (cdr link-pair)))
                (cond
                 ;; Org-roam node: STRIP 'id:' prefix!
                 ((and (string-prefix-p "id:" link)
                       (not (member link seen)))
                  (let* ((real-id (substring link 3))
                         (child-results (my/gptel-org-roam-node-content-by-id real-id (cons link seen) file-seen)))
                    (setq result (append result child-results))))
                 ;; Absolute file path
                 ((and (string-prefix-p "/" link)
                       (not (member link file-seen))
