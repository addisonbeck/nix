...
#+begin_src emacs-lisp
  ;; Define functions for LLM interaction with org-roam
  (defun my/org-roam-node-create-noninteractive (title content)
    "Create an org-roam node with TITLE and CONTENT non-interactively."
  ...
  (defun my/org-roam-node-search (query)
    "Search org-roam nodes matching QUERY and return results as a string."
    (let ((nodes (org-roam-node-list))
          (results '()))
      (dolist (node nodes)
        (when (or (string-match-p query (org-roam-node-title node))
                  (string-match-p query (or (org-roam-node-tags node) "")))
          (push (format "Title: %s\nTags: %s\nFile: %s\n\n"
  ...
  ;; Register GPTel tools for org-roam manipulation
  (with-eval-after-load 'gptel
    (gptel-make-tool
     "create-roam-node"
     `((description . "Create a new org-roam note")
       (function . my/org-roam-node-create-noninteractive)
       (parameters
        (title . ((type . "string")
    ...
    (gptel-make-tool
     "search-roam-nodes"
     `((description . "Search org-roam nodes by title or tags")
       (function . my/org-roam-node-search)
       (parameters
        (query . ((type . "string")
                 (description . "Search query for matching nodes")))))))
#+end_src
