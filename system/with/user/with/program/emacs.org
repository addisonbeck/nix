...
(defun my/gptel-send-one-shot ()
  (interactive)
  (let ((gptel-cache '(system tool)))
    (call-interactively #'gptel-send)))

(defun my/gptel-send-multi-turn ()
  (interactive)
  (let ((gptel-cache t))
    (call-interactively #'gptel-send)))
#+end_src
...
I've got this function for returning an org-roam node by id. It is supposed to support a "Required Reading" section that can be a list of memories or absolute file paths. Absolute file paths aren't working. For example, on this "Memory To Kindle" node, when I run this function on this node I get back the node content itself, as well as the GEB Introduction Quiz node, but none of the file paths. Do you know why? Does it have something to do with the =file:= prefix at the beginning of each file path?

Return a full replacement function in an emacs-lisp source block.
...
#+begin_src emacs-lisp
(defvar my/executer-picker-candidates
