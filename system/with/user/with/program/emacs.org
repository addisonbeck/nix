...
(defun my/search-llm-prompts ()
  "Pick a source block from `my/llm-prompts-file', fill placeholders, and insert at point."
  (interactive)
  ...
  (with-temp-buffer
    ...
    (let ((parse-tree (org-element-parse-buffer))
      ...
      (org-element-map parse-tree 'src-block
        (lambda (src)
          (let* ((value (org-element-property :value src))
                 (parent (org-element-lineage src '(headline) t))
                 (heading (if parent 
                 ...
                 (preview (truncate-string-to-width 
                           ...
                           60 nil nil "…"))
                 (display (format "%s  [%s]" heading preview)))
            (push (cons display value) source-blocks))))
      
      (unless source-blocks
        (user-error "No src-blocks found in %s" my/llm-prompts-file))
      
      ;; Pick one using consult
      (let* ((choice (consult--read (nreverse source-blocks)
                                    :prompt "Select prompt: "
                                    :require-match t))
             (raw-content (cdr (assoc choice source-blocks))))
        
        ;; Insert the content (add placeholder replacement if needed)
        (when raw-content
          ;; If my/replace-prompt-placeholders exists, use it
          (if (fboundp 'my/replace-prompt-placeholders)
              (let ((filled (my/replace-prompt-placeholders raw-content)))
                (when filled (insert filled)))
            ;; Otherwise just insert the raw content
            (insert raw-content)))))))
#+end_src
