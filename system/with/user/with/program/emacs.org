...
;; Fix for calfw truncation alignment issue
(with-eval-after-load 'calfw
  (defun my/cfw:trim-text (text width)
    "Trim TEXT to fit in WIDTH, without adding ellipsis that breaks formatting."
    (if (< width 0) ""
      (if (<= (string-width text) width)
          text
        (truncate-string-to-width text width nil nil nil ""))))
  
  ;; Override the default truncation function
  (advice-add 'cfw:trim-text :override #'my/cfw:trim-text))
#+end_src
