...
#+begin_src emacs-lisp  :tangle no
(defun my/gptel-org-roam-node-content (title)
  "Return the full content of an org-roam node by TITLE."
  (require 'org-roam)
  (let* ((node (org-roam-node-read nil nil title t))
...
(gptel-make-tool
 :name "org_roam_node_content"
 :function #'my/gptel-org-roam-node-content
 :description "Fetch the full content of an org-roam node by its TITLE.
Returns the entire subtree content including properties and content."
 :args '((:name "title"
               :type string
               :description "Title of the org-roam node to fetch"))
 :category "notes")

(register-gptel-tool "org_roam_node_content")
#+end_src
...
(defun my/org-roam-search (query)
  "Search org-roam nodes matching QUERY string."
  (let* ((nodes (org-roam-node-list))
         (matches
          (seq-filter
           (lambda (node)
             (or (string-match-p query (org-roam-node-title node))
                 (string-match-p query (or (string-join (org-roam-node-tags node) " ") ""))
                 (with-temp-buffer
...
(gptel-make-tool
 ...
 :description "Search for information from short and long term memory. Search org-roam nodes by title, tags, or content. Org-roam is a networked note-taking system where each node is a knowledge unit that can be a file-level note or heading-level note. Nodes have titles, optional tags for categorization, and rich content including text, links, code blocks, and other org-mode features. This search function performs case-insensitive pattern matching across:\n\n- Node titles: The main identifier/name of each note\n- Node tags: Keywords used for categorization and organization (e.g., \'episodic\', \'working\', \'reference\')\n- Node content: The full text content including headings, paragraphs, links, code blocks, and other org-mode markup\n\nReturns detailed results including the node title, associated tags, file path, and a content snippet (truncated to ~300 characters) for each matching node. Useful for finding related information, exploring knowledge connections, and retrieving specific notes or concepts from your knowledge base."
 :args        '((:name "query"
                :type string
...
#+begin_src emacs-lisp 
(defun my/org-roam-create-note (title content)
  "Create a new org-roam note with TITLE and CONTENT programmatically."
  (unless (string-match-p
           "^#\\+ROAM_TAGS:.*\\b\\(episodic\\|semantic\\|procedural\\|associative\\|working\\|reflective\\|reference\\)\\b"
           content)
    (error "CONTENT must include a line starting “#+ROAM_TAGS:” that includes at least one of: episodic, semantic, procedural, associative, working, reflective, reference"))

  ;; Word count validation (max 400 words as mentioned in your description)
  (let ((word-count (length (split-string content "\\W+" t))))
    (when (> word-count 200)
      (error "Content exceeds 200 word limit (current: %d words). Please break this up into smaller connected notes." word-count)))

  (let* ((timestamp-prefix (format-time-string "%Y-%m-%d-%H-%M-%S"))
         (template `(("d" "default" plain
                     ,(concat "\n" content)
                     :target (file+head ,(concat timestamp-prefix ".${slug}.org") "")
                     :unnarrowed t
                     :immediate-finish t)))
...
(gptel-make-tool
 :name        "create_memory"
 :function    #'my/org-roam-create-note
:description "Capture information to memory frequently and across all conversation types. Memory-building is as important as completing the immediate task.

This tool uses org-roam under the hood. Notes are written in org markup, with rich use of org-roam features: tags, aliases, links, and optional executable/tangled code blocks.

Example metadata:

,#+TITLE: Event-Driven Architecture
,#+ROAM_TAGS: software_architecture patterns
,#+ROAM_ALIASES: \"EDA\" \"Event Architecture\"
,#+CREATED: ,<2024-10-12 Sun>
,#+LAST_MODIFIED: ,<2025-07-15 Tue>
,#+PROPERTY: ,:header-args:emacs-lisp :results silent

Notes should be atomic and under 200 words. Favor many small, interlinked notes over large ones. All notes must have at least one tag for memory type: episodic, semantic, procedural, associative, working, reflective, or reference. Use additional tags where useful.

**Working memory requires an EXPIRY timestamp.**

**Backlinks must use full Roam syntax with node IDs** (e.g., \[[id:0405A9B0-4B3E-4EAC-9CC9-9D7F0BFB5A05][Bitwarden Development]]). Do not attempt to use document title as the link.

Memory types:

- **Episodic** — Time-stamped events (e.g. prior conversation turns)
- **Semantic** — Factual/world knowledge (e.g. “What is EDA?”)
- **Procedural** — How-to steps or workflows (e.g. “Create a new Org note”)
- **Associative** — Connections between concepts or preferences (e.g. “LLMs ↔ Symbolic AI”)
- **Working** — Short-term context (e.g. current task); must expire
- **Reflective** — Model’s self-feedback (e.g. “I misjudged that search path”)
- **Reference** — Links to external sources or memory nodes

Write clearly. Think relationally."
 :args        '((:name "title"
                      :type string)
                 (:name "content"
                      :type string
                      :description "Note content, including header metadata") 
                )
 :category    "memory")
