...
(setq budget-accounts '(
  "Assets:Checking"
  "Assets:HSA"
  "Liabilities:CreditCard:AppleCard"
  "Liabilities:CreditCard:PrimeStoreCard"
))
(setq budget-categories '(
  "Expenses:Groceries"
  "Expenses:Dining"
...
(defun org-capture-select-payee ()
  (org-completing-read "Payee: " budget-payees))

(defun org-capture-select-account ()
  (org-completing-read "Account: " budget-accounts))

(defun org-capture-select-category ()
  (org-completing-read "Category: " budget-categories))

(add-to-list 'org-capture-templates
             '("b" "Budget transaction" entry
               (file+headline "~/notes/budget.org" "Transactions")
               "** [PENDING] %(progn (setq transaction-date (org-read-date nil nil nil \"Date: \")) transaction-date): $%(progn (setq transaction-amount (read-string \"Amount: \")) transaction-amount) @ %(progn (setq transaction-payee (org-capture-select-payee)) transaction-payee) on %(progn (setq transaction-account (org-capture-select-account)) transaction-account)
%^{Description}
,#+begin_src ledger
%(progn transaction-date) ! %(progn transaction-payee) 
    %(progn (setq transaction-category (org-capture-select-category)) transaction-category)  $%(progn transaction-amount)
    %(progn transaction-account)
,#+end_src
" :empty-lines 1))
#+end_src
...
(run-with-timer 0 (* 10 60) #'my/fetch-github-prs)
#+end_src
