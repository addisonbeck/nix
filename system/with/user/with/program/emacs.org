...
(defun my/wikipedia-search (search-term)
  "Search Wikipedia for SEARCH-TERM and display matching URLs in a buffer."
  (interactive "sSearch Wikipedia for: ")
  (let* ((url-request-method "GET")
         (search-url (format "https://en.wikipedia.org/w/api.php?action=opensearch&search=%s&limit=10&namespace=0&format=json"
                             (url-hexify-string search-term)))
         (buffer (url-retrieve-synchronously search-url)))
    (with-current-buffer buffer
      (goto-char (point-min))
      ...
      (let* ((json-array-type 'list)
             ...
             (data (json-read))
             (results (nth 3 data))) ;; URLs are the 4th item
        (with-output-to-temp-buffer "*Wikipedia Search Results*"
          (if results
              (progn
                (princ (format "Search results for: %s\n\n" search-term))
                (dolist (url results)
                  (princ (format "%s\n" url))))
