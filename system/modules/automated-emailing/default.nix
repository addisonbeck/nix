{
  ...
  calibreBinary =
    if isDarwin
    then "/opt/homebrew/bin/ebook-convert"
    else "${pkgs.calibre}/bin/ebook-convert";
...
in {
  ...
  options.my.wikipedia-to-kindle-generate = lib.mkOption {
    type = lib.types.package;
    default =
      pkgs.writeShellScriptBin "wikipedia-to-kindle-generate"
  ...
  options.my.cookbook-to-kindle-generate = lib.mkOption {
    type = lib.types.package;
    default =
      pkgs.writeShellScriptBin "cookbook-to-kindle-generate"
  ...
  options.my.memory-to-kindle-generate = lib.mkOption {
    type = lib.types.package;
    default =
      pkgs.writeShellScriptBin "memory-to-kindle-generate"
  ...
  config = {
    ...
    environment.etc."Muttrc".text = ''
      ...
      set imap_pass = "`cat ${config.sops.secrets.do-not-reply-email-password.path}`"
      ...
      set smtp_pass = "`cat ${config.sops.secrets.do-not-reply-email-password.path}`"
