...
(defun my/gptel-org-roam-node-content-by-id (id &optional seen file-seen)
  "Returns the full text content of a given memory, as well as any memories or files in that memory's 'Required Reading' section. Required Reading is included so Bobert does not go out and read those files manually."
  (interactive)
            ...
            (save-excursion
              ...
              (let ((case-fold-search t))  ; be lenient about heading case
                (when (re-search-forward "^\\(\\*+\\) Required Reading\\s-*$" nil t)
                  (let* ((level (length (match-string 1)))        ; Count the stars  
                         (start (point))
                         (end-pattern (format "^\\*\\{1,%d\\} " level))  ; Same level or higher
