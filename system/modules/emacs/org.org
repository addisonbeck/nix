...
With \\[universal-argument], exports the 7-day weekly view (agenda command \"w\")."
  ...
  (let* ((weekly (consp arg))
         ...
         (epub-dir (expand-file-name "~/notes/epubs"))
         (epub-file (expand-file-name (concat slug ".epub") epub-dir))
         (tmp-html (make-temp-file "agenda-" nil ".html")))
    (save-window-excursion
      (let ((org-agenda-window-setup 'current-window)
            (org-agenda-sticky nil))
        (org-agenda nil cmd-key)
        (org-agenda-write tmp-html nil t)))
    (make-directory epub-dir t)
    (let ((compilation-buffer-name-function (lambda (_mode) "*agenda-to-epub*")))
      (compile
       (format "pandoc %s -o %s --from=html --to=epub3 --metadata title=%s --toc --standalone"
               (shell-quote-argument tmp-html)
