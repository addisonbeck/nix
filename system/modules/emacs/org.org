    ...
    (let* ((buf-name (my/agenda--buf-name-for-marker marker))
      ...
      (unless (get-buffer buf-name)
        (with-current-buffer (get-buffer-create buf-name)
          ...
          ;; Two-phase dialog confirmation: sending \e[B and \r as one write causes
          ;; Claude to exit (the Enter arrives before the dialog finishes processing
          ;; the down arrow). Split into two timers to mimic natural keypress timing.
          ;; At 5s: send down arrow to move selection from No to Yes.
          ;; At 5.5s: send Enter to confirm Yes.
          (run-with-timer 5 nil
                          (lambda ()
                            (when-let* ((buf (get-buffer buf-name))
                                        (proc (get-buffer-process buf)))
                              (process-send-string proc "\e[B"))))
          (run-with-timer 5.5 nil
                          (lambda ()
                            (when-let* ((buf (get-buffer buf-name))
                                        (proc (get-buffer-process buf)))
