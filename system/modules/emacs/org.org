...
#+begin_src emacs-lisp
  ...
  (defun my/spawn-claude-for-todo (marker heading)
    "Create a vterm buffer for the todo item at MARKER with HEADING text and launch Claude."
    (let* ((buf-name (my/agenda--buf-name-for-marker marker))
           (task-description (format "Work on this task: %s" heading)))
      (unless (get-buffer buf-name)
        (with-current-buffer (vterm buf-name)
          (vterm-send-string
           (format "claude --dangerously-skip-permissions -p %s"
                   (shell-quote-argument task-description)))
          (vterm-send-return)
