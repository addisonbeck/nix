...
#+begin_src emacs-lisp
  ...
  ;; Startup orphan recovery: on Emacs init, find any IN_PROGRESS items in the
  ;; agenda file that have no live vterm buffer and reset them to TODO.
  ;; This handles items left IN_PROGRESS by a previous Emacs daemon session crash.
