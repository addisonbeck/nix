...
Global default for all org files. =bobert-work-agenda.org= overrides this locally with a
three-state machine (TODO → IN_PROGRESS → DONE) via a per-file =#+TODO:= directive.
...
(defun my/get-org-agenda-files ()
  ...
  (list (expand-file-name "logs.org" my-notes-directory)
        ...
        (expand-file-name "roam/inbox.org" my-notes-directory)
        (expand-file-name "roam/mobile-inbox.org" my-notes-directory)
        (expand-file-name "roam/boberts-inbox.org" my-notes-directory)
        (expand-file-name "roam/handwritten-inbox.org" my-notes-directory)
        (expand-file-name "reminders.org" my-notes-directory)
        (expand-file-name "meal-plan.org" my-notes-directory)
        (expand-file-name "contacts.org" my-notes-directory)
        (expand-file-name "code-reviews.org" my-notes-directory)
        (expand-file-name "agenda/bobert-work-inbox.org" my-notes-directory)
        (expand-file-name "agenda/bobert-work-agenda.org" my-notes-directory)))
...
#+begin_src emacs-lisp
(setq my-refile-files 
      (append org-agenda-files 
              (list (expand-file-name "archive.org" my-notes-directory))))
(setq org-refile-targets '((my-refile-files :maxlevel . 2)))
#+end_src
...
#+begin_src emacs-lisp 
  ...
  (setq org-default-notes-file (expand-file-name "roam/inbox.org" my-notes-directory))
...
'(("d" "daily dashboard"
   (
    ...
    (agenda "Schedule and Habits"
		...
		(:name "Platform PRs"
         :and (:deadline today :category "code review" :property ("AUTHOR" "coroiu"))
         :and (:deadline today :category "code review" :property ("AUTHOR" "djsmith85"))
         :and (:deadline today :category "code review" :property ("AUTHOR" "dereknance"))
         :and (:deadline today :category "code review" :property ("AUTHOR" "trmartin4"))
         :and (:deadline today :category "code review" :property ("AUTHOR" "dani-garcia"))
         :and (:deadline past :category "code review" :property ("AUTHOR" "coroiu"))
         :and (:deadline past :category "code review" :property ("AUTHOR" "djsmith85"))
         :and (:deadline past :category "code review" :property ("AUTHOR" "dereknance"))
         :and (:deadline past :category "code review" :property ("AUTHOR" "trmartin4"))
         :and (:deadline past :category "code review" :property ("AUTHOR" "dani-garcia")))
		(:name "Work"
		...
		(:name "Ready To Merge Code Reviews"
        :and (:deadline today :category "ready-to-merge")
        :and (:deadline past :category "ready-to-merge"))
		(:name "Code Review Bunker"
        :and (:deadline today :category "code review bunker")
        :and (:deadline past :category "code review bunker"))
		(:name "Community PRs"
        :and (:deadline today :category "community-prs")
        :and (:deadline past :category "community-prs"))
		(:name "Renovate PRs"
        :and (:deadline today :category "renovate-prs")
        :and (:deadline past :category "renovate-prs"))
...
#+begin_src emacs-lisp
(defun my/org-agenda-to-epub (&optional arg)
  "Export the org-agenda to an EPUB saved in ~/notes/epubs.
Without a prefix, exports the daily dashboard (agenda command \"d\").
With \\[universal-argument], exports the 7-day weekly view (agenda command \"w\")."
  (interactive "P")
  (require 'org-agenda)
  (let* ((weekly (consp arg))
         (cmd-key (if weekly "w" "d"))
         (date-str (format-time-string "%Y-%m-%d"))
         (title (if weekly
                    (format "Weekly Agenda — %s" date-str)
                  (format "Daily Agenda — %s" date-str)))
         (tmp-org (make-temp-file "agenda-" nil ".org")))
    (save-window-excursion
      (let ((org-agenda-window-setup 'current-window)
            (org-agenda-sticky nil))
        (org-agenda nil cmd-key)
        (org-agenda-write tmp-org nil t)))
    ;; Prepend #+TITLE: so memory-to-epub-file can find a title;
    ;; org-agenda-write emits bare subtrees with no file header.
    (let ((subtrees (with-temp-buffer
                      (insert-file-contents tmp-org)
                      (buffer-string))))
      (with-temp-file tmp-org
        (insert "#+TITLE: " title "\n\n")
        (insert subtrees)))
    (let ((compilation-buffer-name-function (lambda (_mode) "*agenda-to-epub*")))
      (compile (format "memory-to-epub-file %s" (shell-quote-argument tmp-org)) t))))
#+end_src

#+begin_src emacs-lisp
(defun my/org-agenda-to-pdf (&optional arg)
  "Export the org-agenda to a print-ready PDF saved in ~/notes/epubs.
Without a prefix, exports the daily dashboard (agenda command \"d\").
With \\[universal-argument], exports the 7-day weekly view (command \"w\").
Renders with white background and colored text suitable for stylus annotation."
  (interactive "P")
  (require 'org-agenda)
  (require 'htmlize)
  (let* ((weekly (consp arg))
         (cmd-key (if weekly "w" "d"))
         (date-str (format-time-string "%Y-%m-%d"))
         (slug (format "agenda-%s%s" (if weekly "week-" "") date-str))
         (pdf-file (expand-file-name (concat slug ".pdf")
                                     (expand-file-name "~/notes/pdfs")))
         (tmp-html (make-temp-file "agenda-pdf-" nil ".html"))
         ...
         (org-agenda-export-html-style
          (concat
           ...
           "  pre  { margin: 0; padding: 1.5em 2em; white-space: pre-wrap;\n"
           "         word-break: break-word; font-family: inherit;\n"
           "         font-size: 13px; line-height: 1.85;\n"
           "         background: white; color: #282828; }\n"
           "  .default                     { color: #282828; }\n"
           "  .org-agenda-date-today       { color: #d65d0e; font-weight: bold; font-size: 1.1em; }\n"
           "  .org-agenda-date             { color: #458588; font-weight: bold; }\n"
           "  .org-agenda-structure        { color: #689d6a; font-weight: bold; }\n"
           "  .org-super-agenda-header     { color: #7c6f64; font-weight: bold; }\n"
           "  .org-todo                    { color: #cc241d; font-weight: bold; }\n"
           "  .org-done                    { color: #928374; }\n"
           "  .org-priority                { color: #b57614; font-weight: bold; }\n"
           "  .org-scheduled-today         { color: #076678; font-weight: bold; }\n"
           "  .org-scheduled               { color: #458588; }\n"
           "  .org-scheduled-previously    { color: #9d0006; }\n"
           "  .org-upcoming-deadline       { color: #b57614; }\n"
           "  .org-tag                     { color: #427b58; font-size: 0.9em; }\n"
           "  .org-time-grid               { color: #bdae93; }\n"
           "  .org-agenda-dimmed-todo-face { color: #928374; }\n"
    ...
    (save-window-excursion
      (let ((org-agenda-window-setup 'current-window)
            (org-agenda-sticky nil))
        (org-agenda nil cmd-key)
        (org-agenda-write tmp-html nil t)))
    (make-directory (file-name-directory pdf-file) t)
    (let ((compilation-buffer-name-function (lambda (_mode) "*agenda-to-pdf*")))
      (compile
       (format "google-chrome --headless --no-sandbox --print-to-pdf=%s --print-to-pdf-no-header %s"
               (shell-quote-argument pdf-file)
               (shell-quote-argument (concat "file://" tmp-html)))
       t))))
#+end_src

* Org Clock
#+begin_src emacs-lisp
;; Don't persist clocks from conversation files
(defun my/org-clock-should-persist-p ()
  "Return nil if current buffer is a conversation file."
  (not (string-match-p "\\.conversation-" (or (buffer-file-name) ""))))

;; Override org-clock persistence for conversation files
(advice-add 'org-clock-save :before-while #'my/org-clock-should-persist-p)

;; Don't try to resolve clocks in conversation files  
(setq org-clock-resolve-expert t)

;; More restrictive about what constitutes valid clock data
(setq org-clock-persist 'history) ;; Only persist history, not active clocks
#+end_src
...
#+begin_src emacs-lisp
  (with-eval-after-load 'transient
    (transient-append-suffix 'my/search-menu "n"
      '("q" "Query Roam" org-roam-ql-search)))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/org-mode-set-keybindings ()
    ...
    ;; Terminal/SSH fixes: C-i (TAB) is stolen by evil-jump-forward over SSH,
    ;; and C-<return> can't be transmitted by terminals.
    (evil-define-key 'normal org-mode-map (kbd "TAB") 'org-cycle)
    (evil-define-key '(normal insert) org-mode-map (kbd "C-k") #'my/insert-menu))
#+end_src
...
(defun my/org-copy-heading-path ()
  "Search through all org headings and copy the selected heading's full path.
The path includes all parent headings from the selected heading outward,
separated by ' > '."
  (interactive)
  ...
  (let ((headings '()))
    ;; Collect all headings with their paths
    (org-map-entries
     (lambda ()
    ...
    (if headings
        (let* ((choice (completing-read "Select heading: " 
                                       (mapcar #'car (nreverse headings)) 
                                       nil t))
               (selected-path choice))
          (kill-new selected-path)
          (message "Copied: %s" selected-path))
...
#+begin_src emacs-lisp
(defun my/refile-log-to-date-node ()
  "Refile current heading to corresponding 'Log Date' org-roam node.
Extracts date from heading format [YYYY-MM-DD ...] and refiles to node
titled 'Log Date YYYY-MM-DD'. Creates the node if it doesn't exist.
Refiled entries become top-level headings in the target file."
  (interactive)
  (unless (org-at-heading-p)
    (org-back-to-heading t))
  
  (let* ((heading (org-get-heading t t t t))
         ;; Extract date from heading [YYYY-MM-DD ...] (allows day of week and time)
         (date-match (string-match "\\[\\([0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\}\\)[^]]*\\]" heading))
         (date-str (when date-match (match-string 1 heading))))
    
    (unless date-str
      (user-error "No date found in heading. Expected format: [YYYY-MM-DD ...]"))
    
    (let* ((target-title (concat "Log Date " date-str))
           ;; Find the node with matching title
           (node (seq-find
                  (lambda (n) (string= (org-roam-node-title n) target-title))
                  (org-roam-node-list))))
      
      ;; Create node if it doesn't exist
      (unless node
        (let* ((id (org-id-new))
               (filename (concat date-str ".log.org"))
          ...
          (with-temp-file filepath
            (insert ":PROPERTIES:\n")
            (insert (format ":ID: %s\n" id))
            (insert ":ROAM_TAGS: episodic daily log\n")
            (insert (format ":CREATED: %s\n" timestamp))
            (insert (format ":LAST_MODIFIED: %s\n" timestamp))
            (insert ":END:\n")
            (insert (format "#+TITLE: %s\n\n" target-title)))
          ...
          ;; Reload the node list and find our new node
          (setq node (seq-find
                      (lambda (n) (string= (org-roam-node-title n) target-title))
                      (org-roam-node-list)))
          
          (message "Created new node: %s" target-title)))
      
      (let* ((target-file (org-roam-node-file node))
             (node-level (org-roam-node-level node))
             ;; Construct refile target - use nil position for file-level nodes
             (refile-target 
              (if (= node-level 0)
                  ;; For file-level nodes, use nil position to refile to file level
                  (list target-title target-file nil nil)
                ;; For heading-level nodes, find the heading by ID
                (let ((target-marker 
                       (with-current-buffer (find-file-noselect target-file)
                         (org-with-wide-buffer
                          ...
                          (when (re-search-forward 
                                 (format "^[ \t]*:ID:[ \t]+%s" 
                                         (regexp-quote (org-roam-node-id node))) 
                                 nil t)
                            (org-back-to-heading t)
                            (point-marker))))))
                  (unless target-marker
                    (user-error "Could not find node location in file"))
                  (list target-title target-file nil target-marker)))))
        
        ;; Perform the refile
        (org-refile nil nil refile-target)
        (message "Refiled to: %s" target-title)))))
#+end_src

#+begin_src emacs-lisp
(with-eval-after-load 'transient
  (transient-append-suffix 'my/refile-menu "r"
    '("l" "Today's Log" my/refile-log-to-date-node)))
#+end_src

* Bobert Agenda Watcher

Loop 2 implementation: polls bobert-work-agenda.org every 120 seconds for TODO items,
dispatches each to Claude Code via vterm, and transitions state via process sentinel.
State machine: TODO -> IN_PROGRESS -> DONE (clean exit) | TODO (error exit).
See ADR-015 (Emacs timer), ADR-016 (state machine).

#+begin_src emacs-lisp
  (defvar my/agenda-watcher-timer nil
    "Timer object for the Bobert agenda watcher loop. Stored to allow cancellation.")

  (defvar my/agenda-file
    (expand-file-name "agenda/bobert-work-agenda.org"
                      my-notes-directory)
    "Absolute path to bobert-work-agenda.org.")

  (defun my/agenda--get-todo-items ()
    "Return a list of (marker . heading-text) for all TODO-state items in the agenda file."
    (let ((results '()))
      (with-current-buffer (find-file-noselect my/agenda-file)
        (org-with-wide-buffer
         (goto-char (point-min))
         (while (re-search-forward org-heading-regexp nil t)
           (when (string= (org-get-todo-state) "TODO")
             (push (cons (point-marker) (org-get-heading t t t t))
                   results)))))
      (nreverse results)))

  (defun my/agenda--set-todo-state (marker state)
    "Set the TODO state of the heading at MARKER to STATE."
    (with-current-buffer (marker-buffer marker)
      (goto-char marker)
      (org-todo state)
      (save-buffer)))

  (defun my/agenda--mark-in-progress (marker)
    "Transition item at MARKER from TODO to IN_PROGRESS."
    (my/agenda--set-todo-state marker "IN_PROGRESS"))

  (defun my/agenda--buf-name-for-marker (marker)
    "Return the vterm buffer name used for the item at MARKER."
    (with-current-buffer (marker-buffer marker)
      (goto-char marker)
      (let ((id (org-entry-get nil "ID")))
        (format "claude-todo-%s" (or id (number-to-string (marker-position marker)))))))

  (defun my/agenda--mark-todo-done (buf-name)
    "Find the agenda item whose vterm buffer is BUF-NAME and mark it DONE."
    (with-current-buffer (find-file-noselect my/agenda-file)
      (org-with-wide-buffer
       (goto-char (point-min))
       (while (re-search-forward org-heading-regexp nil t)
         (when (string= (org-get-todo-state) "IN_PROGRESS")
           (let* ((id (org-entry-get nil "ID"))
                  (expected-buf (format "claude-todo-%s" (or id ""))))
             (when (string= expected-buf buf-name)
               (org-todo "DONE")
               (save-buffer))))))))

  (defun my/agenda--reset-todo-to-todo (buf-name)
    "Find the agenda item whose vterm buffer is BUF-NAME and reset it to TODO."
    (with-current-buffer (find-file-noselect my/agenda-file)
      (org-with-wide-buffer
       (goto-char (point-min))
       (while (re-search-forward org-heading-regexp nil t)
  ...
  (defun my/agenda--watch-buffer-for-completion (process event)
    "Process sentinel for vterm buffers spawned by my/spawn-claude-for-todo.
  On clean exit (finished), marks the corresponding agenda item DONE.
  On any other event (error, signal), resets the item to TODO for retry."
    (let ((buf-name (buffer-name (process-buffer process))))
      (cond
       ((string= event "finished\n")
        (message "[agenda-watcher] %s: finished cleanly, marking DONE" buf-name)
        (my/agenda--mark-todo-done buf-name))
       (t
        (message "[agenda-watcher] %s: exited with event %s, resetting to TODO" buf-name event)
        (my/agenda--reset-todo-to-todo buf-name)))))

  (defun my/spawn-claude-for-todo (marker heading)
    "Create an interactive vterm buffer for the TODO at MARKER and launch Claude.
  Uses --permission-mode bypassPermissions (no confirmation dialog) and passes
  the task body as a positional argument so Claude starts working immediately."
    (let* ((buf-name (my/agenda--buf-name-for-marker marker))
           (project-dir (with-current-buffer (marker-buffer marker)
                          (goto-char marker)
                          (org-entry-get nil "PROJECT_DIR")))
           (task-body (with-current-buffer (marker-buffer marker)
                        (goto-char marker)
      ...
      (unless (get-buffer buf-name)
        (with-current-buffer (get-buffer-create buf-name)
          ...
          (when (and project-dir (not (string-empty-p project-dir)))
            (vterm-send-string (format "cd %s" (shell-quote-argument project-dir)))
            (vterm-send-return))
          (vterm-send-string (format "claude --permission-mode bypassPermissions %s"
                                     (shell-quote-argument task-body)))
          (vterm-send-return)
          (when-let ((proc (get-buffer-process (current-buffer))))
            (set-process-sentinel proc #'my/agenda--watch-buffer-for-completion))
          ;; Poll the buffer every second until the bypass dialog appears ("Enter to
          ;; confirm"), then send down arrow + Enter to select "Yes, I accept".
          ;; Two-phase send: \e[B and \r must be split (sending together causes Claude
          ;; to exit before the dialog finishes processing the down arrow).
          ;; A 30s timeout guard cancels the poller if the dialog never appears.
          (let* ((poll-timer (run-with-timer
                              1 1
                              (lambda (self)
                                (when-let* ((buf (get-buffer buf-name))
                                            (proc (get-buffer-process buf))
                                            (_ (with-current-buffer buf
                                                 (string-match-p "Enter to confirm"
                                                                  (buffer-string)))))
                                  (cancel-timer self)
                                  (process-send-string proc "\e[B")
  ...
  (defun my/agenda-watcher ()
    "Poll bobert-work-agenda.org for TODO items and dispatch each to Claude via vterm.
  Only dispatches items in TODO state. Transitions item to IN_PROGRESS before spawning."
    (condition-case err
        (let ((items (my/agenda--get-todo-items)))
          (dolist (item items)
            (let ((marker (car item))
                  (heading (cdr item)))
              (my/agenda--mark-in-progress marker)
              (my/spawn-claude-for-todo marker heading))))
      (error
       (message "[agenda-watcher] error during watcher run: %s" (error-message-string err)))))

  (defun my/start-agenda-loop ()
    "Start the Bobert agenda watcher loop, firing every 120 seconds.
  Idempotent: cancels any existing timer before starting a new one."
    (interactive)
    (when my/agenda-watcher-timer
      (cancel-timer my/agenda-watcher-timer))
    (setq my/agenda-watcher-timer
          (run-with-timer 0 120 #'my/agenda-watcher))
    (message "[agenda-watcher] loop started (120s interval)"))

  (defun my/stop-agenda-loop ()
    "Stop the Bobert agenda watcher loop."
    (interactive)
    (when my/agenda-watcher-timer
      (cancel-timer my/agenda-watcher-timer)
      (setq my/agenda-watcher-timer nil)
      (message "[agenda-watcher] loop stopped")))

  ;; Startup orphan recovery disabled: was resetting IN_PROGRESS items to TODO on
  ;; every Emacs restart, preventing the watcher from ever seeing items in TODO state.
  ;; (add-hook 'after-init-hook
  ;;           (lambda ()
  ;;             (when (file-exists-p my/agenda-file)
  ;;               (with-current-buffer (find-file-noselect my/agenda-file)
  ;;                 (org-with-wide-buffer
  ;;                  (goto-char (point-min))
  ;;                  (while (re-search-forward org-heading-regexp nil t)
  ;;                    (when (string= (org-get-todo-state) "IN_PROGRESS")
  ;;                      (let* ((id (org-entry-get nil "ID"))
  ;;                             (buf-name (format "claude-todo-%s" (or id ""))))
  ;;                        (unless (get-buffer buf-name)
  ;;                          (org-todo "TODO")
  ;;                          (save-buffer)
  ;;                          (message "[agenda-watcher] orphan recovery: reset %s to TODO" buf-name)))))))))))
