    ...
    (let* ((buf-name (my/agenda--buf-name-for-marker marker))
      ...
      (unless (get-buffer buf-name)
        (with-current-buffer (vterm buf-name)
          ...
          ;; At 3s: navigate bypass-permissions dialog to "Yes" and confirm.
          ;; Uses process-send-string with raw escape sequences (\e[B = down arrow)
          ;; rather than vterm-send-key, which is unreliable inside timers.
          ;; The task body is already on the command line so no further injection needed.
          (run-with-timer 3 nil
                          (lambda ()
                            (when-let* ((buf (get-buffer buf-name))
                                        (proc (get-buffer-process buf)))
                              (process-send-string proc "\e[B\r"))))))))
