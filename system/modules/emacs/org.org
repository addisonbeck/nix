...
#+begin_src emacs-lisp
  ...
  Starts Claude interactively (no --print), cds to PROJECT_DIR first if set,
  then sends the task description as the opening message after a short delay."
    (let* ((buf-name (my/agenda--buf-name-for-marker marker))
      ...
      (unless (get-buffer buf-name)
        (with-current-buffer (vterm buf-name)
          ...
          ;; At 1.5s: navigate to "Yes" in the bypass-permissions dialog
          ;;   (default is No, so we need <down> to select Yes, then Enter).
          ;; At 6s: send task body via process-send-string (writes directly
          ;;   to the pty, more reliable than vterm-send-string for injecting
          ;;   text into a running interactive process).
          ...
          (run-with-timer 6 nil
                          (lambda ()
                            (when-let* ((buf (get-buffer buf-name))
                              ...
                              (process-send-string proc (concat task-body "\n")))))))))
