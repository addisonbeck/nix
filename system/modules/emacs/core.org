...
#+begin_src emacs-lisp
  ...
  (use-package dashboard
    ...
    (setq dashboard-navigator-buttons
	  `(;; Row 1: Navigation
	    ((,(nerd-icons-octicon "nf-oct-stack" :height 1.2 :v-adjust 0.0)
	      "Buffers"
	      "Switch buffer"
	      (lambda (&rest _) (consult-buffer)))
	     (,(nerd-icons-octicon "nf-oct-search" :height 1.2 :v-adjust 0.0)
	      "Find File"
	      "Find file in project"
	      (lambda (&rest _) (projectile-find-file)))
	     (,(nerd-icons-octicon "nf-oct-database" :height 1.2 :v-adjust 0.0)
	      "Memory"
	      "Search org-roam memory"
	      (lambda (&rest _) (org-roam-node-find)))
	     (,(nerd-icons-octicon "nf-oct-git_branch" :height 1.2 :v-adjust 0.0)
	      "Magit"
	      "Open project in Magit"
	      (lambda (&rest _) (my/projectile-magit-status))))
	    ;; Row 2: Actions
	    ((,(nerd-icons-octicon "nf-oct-copilot" :height 1.2 :v-adjust 0.0)
	      "Claude"
	      "Create Claude Code buffer"
	      (lambda (&rest _) (my/create-claude-code-buffer)))
	     (,(nerd-icons-octicon "nf-oct-calendar" :height 1.2 :v-adjust 0.0)
	      "Agenda"
	      "Open org-agenda"
	      (lambda (&rest _) (org-agenda)))
	     (,(nerd-icons-octicon "nf-oct-pencil" :height 1.2 :v-adjust 0.0)
	      "Capture"
	      "Org capture"
...
* Mobile Mode for Phone/SSH Usage
Mobile mode provides better ergonomics when using Emacs over SSH on mobile devices by increasing font size and setting appropriate terminal width.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun my/insert-org-link-to-notes ()
    "Select a file from the notes directory and insert an org link at point."
    (interactive)
    (require 'org)
    (let* ((notes-dir (expand-file-name (my/get-project-path "notes")))
	   (files (directory-files-recursively notes-dir ""))
	   (relative (mapcar (lambda (f) (file-relative-name f notes-dir)) files))
	   (selected (completing-read "Note: " relative nil t))
	   (abs (expand-file-name selected notes-dir))
	   (link (org-link-make-string
		  (concat "file:" (file-truename abs))
		  (file-name-nondirectory abs))))
      (if (derived-mode-p 'vterm-mode)
...
#+begin_src emacs-lisp
  (transient-define-prefix my/insert-menu ()
			   ...
			   ["Insert"
			    ...
			    ("n" "Notes Link" my/insert-org-link-to-notes)
...
#+begin_src emacs-lisp
  (defun my/search-notes ()
    "Recursively find a file in the notes directory with fuzzy completion."
    (interactive)
    (let* ((notes-dir (expand-file-name (my/get-project-path "notes")))
	   (files (directory-files-recursively notes-dir ""))
	   (relative (mapcar (lambda (f) (file-relative-name f notes-dir)) files))
...
#+begin_src emacs-lisp
  ...
  (evil-define-key 'normal 'global (kbd "C-a") 'delete-other-windows)
...
*** Magit Revision Keymaps

#+begin_src emacs-lisp
  (defun my/magit-revision-mode-set-keybindings ()
    (interactive)
    "Restore Avy 's' binding in magit-revision-mode"
...
#+begin_src emacs-lisp
  (defun magit-revision-mode-init ()
    "Function to run on magit revision mode init"
    (my/magit-revision-mode-set-keybindings))
  (add-hook 'magit-revision-mode-hook #'magit-revision-mode-init)
#+end_src
...
#+begin_src emacs-lisp
  (defun my/kill-daemon ()
    "Save all buffers and kill the running Emacs daemon."
    (interactive)
    (save-some-buffers t)
    (server-force-delete)
    (kill-emacs))
