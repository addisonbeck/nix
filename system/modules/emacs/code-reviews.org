...
(let ((memory-result (my/create-memory
  ...
  (condition-case review-file-err
	...
	(with-current-buffer (find-file-noselect memory-file)
	  (goto-char (point-max))
	  (insert (concat "* TODO CR: " pr-title "\n"
			  "** TODO Initial Review\n"
			  "*** Goal\n"
			  "Develop an understanding of the changes presented in this PR\n"
			  "*** Prompt\n"
        "Use the =gh= cli to explore this PR. Read the description, the diff, and any inline or general comments. If the PR is a dependency update load the review-dependency-update skill for delegation instructions. If the PR is not a depdency update simply explain the changes proposed, and write an initial code review. Save the code review to memory, and add it to a top level Required Reading section in this memory.\n"
			  "** TODO Quiz!\n"
			  "*** Goal\n"
			  "Ensure the LLM and the human both really understand these changes\n"
			  "*** Prompt\n"
...
that are prepended to the command as inline shell environment variables."
    (let* ((buf-name (format "*PR-Review-%s*" pr-number))
           (buf (get-buffer buf-name))
           (env-prefix (if env-vars
                           (concat (mapconcat #'identity env-vars " ") " ")
                         ""))
           (full-command (concat env-prefix command)))
      (if buf
          ;; Reuse existing buffer
          (with-current-buffer buf
            (vterm-send-string (concat "clear && " full-command))
            (vterm-send-return))
        ;; Create new buffer
        (vterm buf-name)
...
#+begin_src emacs-lisp :tangle code-reviews.el
  ...
  (defun my/pr-open-in-browser ()
    "Open PR in default web browser."
    (interactive)
    (let* ((ctx (my/pr-detect-context))
           (pr-url (plist-get ctx :pr-url)))
      (if pr-url
          (browse-url pr-url)
...
#+begin_src emacs-lisp :tangle code-reviews.el
  ...
  (defun my/pr-merge ()
    "Merge PR using gh CLI interactive flow."
    (interactive)
    (let* ((ctx (my/pr-detect-context))
           (pr-number (plist-get ctx :pr-number))
           (pr-url (or (plist-get ctx :pr-url) pr-number)))
      (my/pr-review-in-vterm pr-number
                             (format "gh pr merge %s" pr-url))))
#+end_src
