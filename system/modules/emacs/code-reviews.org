...
#+begin_src emacs-lisp :tangle code-reviews.el
  (transient-define-prefix my/pr-review-menu ()
		     ...
		     ("e" "Save as epub" my/pr-save-as-epub)]
			    ["Actions"
...
#+begin_src emacs-lisp :tangle code-reviews.el
  (defun my/pr-save-as-epub ()
    "Export the current PR as an EPUB to ~/notes/epubs using pr-to-epub."
    (interactive)
    (let* ((ctx (my/pr-detect-context))
           (pr-url (plist-get ctx :pr-url))
           (pr-number (plist-get ctx :pr-number)))
      (unless pr-url
        (user-error "No PR URL found"))
      (let* ((buf-name (format "*PR-EPUB-%s*" pr-number))
             (output-dir (expand-file-name "~/notes/epubs/")))
        (make-directory output-dir t)
        (message "Generating EPUB for PR %s..." pr-number)
        (let ((proc (start-process-shell-command
                     buf-name buf-name
                     (concat "pr-to-epub " (shell-quote-argument pr-url)))))
          (set-process-sentinel
           proc
           (lambda (p _event)
             (when (eq (process-status p) 'exit)
               (if (= (process-exit-status p) 0)
                   (message "PR EPUB saved to ~/notes/epubs/")
                 (progn
                   (pop-to-buffer buf-name)
                   (message "EPUB generation failed — see buffer %s" buf-name))))))
          (display-buffer buf-name '(display-buffer-at-bottom . ((window-height . 0.2))))))))
#+end_src
