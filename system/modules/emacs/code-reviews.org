...
** CLI/TUI PR Review Workflow

This section implements terminal-based PR review workflows using gh CLI and gh-dash tools rendered in vterm buffers.

*** Context Detection

#+begin_src emacs-lisp :tangle code-reviews.el
  (defun my/pr-context-from-org-property ()
    "Extract PR number from org-mode PR_URL property.
	    Returns PR number as string or nil if not in org-mode or PR_URL not found."
    (when (derived-mode-p 'org-mode)
      (when-let* ((pr-url (org-entry-get nil "PR_URL"))
		  (pr-num (string-match "github\\.com/.*/pull/\\([0-9]+\\)" pr-url)))
	(match-string 1 pr-url))))

  (defun my/pr-context-from-url-at-point ()
    "Extract PR number from GitHub URL at point.
	    Returns PR number as string or nil if no URL at point or URL not a GitHub PR."
    (when-let* ((url (thing-at-point-url-at-point))
		(pr-num (string-match "github\\.com/.*/pull/\\([0-9]+\\)" url)))
      (match-string 1 url)))

  (defun my/pr-context-from-prompt ()
    "Prompt user for PR number.
	    Returns PR number as string from user input."
    (read-string "Enter PR number: "))

  (defun my/pr-detect-context ()
    "Detect PR number from context with priority chain.
	    Priority: org-property -> URL-at-point -> prompt.
	    Returns PR number as string."
...
#+begin_src emacs-lisp :tangle code-reviews.el
  (defun my/pr-review-start ()
    ...
    (interactive)
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh pr view %s" pr-number))))
  ...
  (defun my/pr-files ()
    ...
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh pr view %s --json files --jq '.files[].path'" pr-number))))
  ...
  (defun my/pr-comments ()
    ...
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh pr view %s --comments" pr-number))))
#+end_src
...
#+begin_src emacs-lisp :tangle code-reviews.el
  (defun my/pr-add-comment ()
    ...
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh pr comment %s" pr-number))))
  ...
  (defun my/pr-submit-review ()
    ...
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh pr review %s" pr-number))))
#+end_src
...
#+begin_src emacs-lisp :tangle code-reviews.el
  (defun my/pr-checks ()
    ...
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh pr checks %s" pr-number))))
  ...
  (defun my/pr-review-history ()
    ...
    (let ((pr-number (my/pr-detect-context)))
      (my/pr-review-in-vterm pr-number
			     (format "gh dash --pr %s" pr-number))))
#+end_src

*** Integration Documentation

This CLI/TUI PR review workflow provides terminal-based PR review capabilities
within Emacs vterm buffers, combining gh CLI and gh-dash tools.

**** Usage

Press =C-o= in any buffer to open the PR review menu. The menu provides 8 workflows:

- Review :: View PR details, diffs, files, and comments
- Actions :: Add comments and submit reviews interactively
- Status :: Check CI/CD results and review timeline

**** Context Detection

PR context is automatically detected from:
1. org-mode =PR_URL= property (for task tracking)
2. GitHub PR URL at point (for ad-hoc reviews)
3. User prompt (fallback)

**** Buffer Management

Each PR gets a dedicated vterm buffer (=*PR-Review-<NUMBER>*=) that persists
across workflow invocations. This allows switching between multiple PRs while
maintaining separate terminal states.

**** Dependencies

- =gh= CLI tool (GitHub CLI)
- =gh-dash= extension (interactive PR dashboard)
- =delta= pager (syntax-highlighted diffs)
- =vterm.el= (terminal emulation)

**** Example Workflow

1. Navigate to org-mode TODO with =PR_URL= property
2. Press =C-o= to open review menu
3. Press =d= to view diff with delta rendering
4. Press =c= to read existing comments
5. Press =a= to add your comment
6. Press =s= to submit review
