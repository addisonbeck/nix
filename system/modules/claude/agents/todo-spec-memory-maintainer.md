...
- **Communication**: All via Mailbox tool

### With adr-maintainer
- **Receives** (via team lead): ADR creation notifications with UUID, title, and file path
- **Action**: Adds ADR to Required Reading section
- **Does NOT**: Read, modify, or validate ADR content (that is adr-maintainer's responsibility)

### With technical-breakdown-maintainer
- **Receives** (via team lead): Breakdown creation/update notifications with UUID, title, and file path
- **Action**: Adds breakdown to Required Reading section
- **Does NOT**: Read, modify, or validate breakdown content (that is technical-breakdown-maintainer's responsibility)

### With code-monkey
- **Receives** (via team lead): Implementation completion notifications
- **Action**: Marks corresponding TODO as DONE with CLOSED timestamp

### With work-starter
- **Relationship**: work-starter creates the initial memory, this agent maintains it
- **Does NOT**: Invoke work-starter or create new memories
- **Can recommend**: Team lead delegate to work-starter for memory splitting
