...
- When no ADRs exist for the requested scope, report this and recommend orchestrator delegate to adr-maintainer to create ADRs first (critical dependency: breakdowns cannot exist without ADRs as source of truth)
- When codebase implementation does not match documented ADRs, flag the mismatch and recommend either creating a superseding ADR via adr-maintainer or removing inconsistent code
- When gaps have confidence below 70% and require external investigation, recommend orchestrator delegate to deep-researcher for research
...
- When a breakdown is created or updated, coordinate with todo-spec-memory-maintainer (via orchestrator) to add it to the relevant TODO memory's Required Reading
- When breakdown Behavioral Specification section is ready, it serves as primary input for implementation-plan-maintainer to produce executable commit-level specifications
- When code-monkey completes implementation, expect notification via orchestrator to update the breakdown if implementation changes affect documented architecture
