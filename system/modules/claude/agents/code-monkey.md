...
- When modifying a trait definition, ALWAYS search for ALL implementations (`rg "impl.*TraitName"`) and update production code + test mocks + examples in the same atomic commit (see Trait Modification Completeness Protocol)
...
## Trait Modification Completeness Protocol

**CRITICAL**: When modifying trait definitions (adding/removing methods, changing signatures, adding annotations like `async_trait`), ALL implementations must be updated atomically in the same commit. Test compilation failures caused by incomplete trait updates are a common process failure.

**Detection Trigger**: If the spec involves modifying a trait definition (interface, protocol, abstract base class, etc.), activate this protocol BEFORE implementation.

**Search Process**:

1. **Identify trait name** from spec (e.g., `Repository`, `AuthProvider`, `DataSource`)

2. **Search for ALL implementations** using language-specific patterns:

   **Rust:**
   ```bash
   # Find all struct implementations of the trait
   rg "impl.*Repository" --files-with-matches

   # Find trait definition and usage
   rg "trait Repository" --files-with-matches
   ```

   **TypeScript/JavaScript:**
   ```bash
   # Find class implementations
   rg "implements.*AuthProvider" --files-with-matches

   # Find interface definition
   rg "interface AuthProvider" --files-with-matches
   ```

   **Python:**
   ```bash
   # Find class implementations
   rg "class.*\(.*DataSource.*\)" --files-with-matches

   # Find abstract base class
   rg "@abstractmethod|class.*DataSource" --files-with-matches
   ```

3. **Generate implementation checklist** categorizing by type:
   ```
   Trait: Repository
   Implementations found:

   Production:
   - src/storage/postgres_repository.rs
   - src/storage/sqlite_repository.rs

   Tests:
   - tests/mock_repository.rs
   - tests/integration_test.rs (inline mock)

   Examples:
   - examples/basic_usage.rs

   Total: 5 implementations requiring updates
   ```

4. **Add all implementation files to "Files to Modify"** if not already present:
   - If spec already lists all implementations → proceed
   - If spec is missing implementation files → ESCALATE with checklist:
   ```
   ESCALATION: Trait modification incomplete.

   The spec modifies trait `Repository` but does not list all implementations for update.
   ...
   All implementations must be updated atomically when trait definitions change.
   Please update spec to include all implementation files.
   ```

5. **During implementation, update ALL implementations** before proceeding to assertions:
   - Update trait definition
   - Update production implementations
   - Update test mocks (CRITICAL - often forgotten)
   - Update example code
   - Verify completeness before running assertions

**Rationale**: When a trait gains `async_trait`, changes method signatures, or adds/removes methods, ALL implementations must reflect these changes. Test mocks are especially prone to being forgotten, leading to test compilation failures that slip through `cargo check` (which doesn't compile test code).

**Evidence**: PM-27126 Chunk 1b added `async_trait` to `Repository` trait but didn't update test mock implementations, causing test compilation failures discovered only during `cargo test --no-run`.
...
## Common Patterns: Rust Trait Objects

**Context**: When working with trait objects (e.g., `Box<dyn Trait>`, `Arc<dyn Trait>`), several Rust-specific integration patterns are required that differ from concrete types.

**Pattern: Debug Implementation for Structs with Trait Object Fields**

Trait objects don't automatically derive `Debug`. When adding a trait object field to a struct with `#[derive(Debug)]`:

1. **Remove `#[derive(Debug)]`** and manually implement Debug
2. **Use placeholder pattern** for trait object fields:
   ```rust
   impl std::fmt::Debug for MyStruct {
       fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
           f.debug_struct("MyStruct")
               .field("name", &self.name)
               .field("provider", &self.provider.as_ref().map(|_| "<ProviderPresent>"))
               .finish()
       }
   }
   ```

**Pattern: Clone via Arc**

Trait objects cannot derive `Clone` directly. Use `Arc` for shared ownership:
```rust
#[derive(Clone)] // Works because Arc<dyn Trait> implements Clone
pub struct Client {
    provider: Arc<dyn AuthProvider>,
}
```

**Pattern: Send + Sync Bounds**

For trait objects used in async contexts or across threads, add bounds explicitly:
```rust
...
// Usage in struct:
client: Arc<dyn Repository> // Repository already requires Send + Sync
```

**Evidence**: PM-27126 Chunk 4 encountered unexpected compilation failure when `InternalClient` gained a trait object field. The struct had `#[derive(Debug)]` which failed because `Box<dyn BrowserProvider>` doesn't implement Debug. Manual Debug implementation with `.map(|_| "<ProviderPresent>")` pattern resolved the issue.

**When to Use**: Any time a spec involves adding trait object fields (`Box<dyn T>`, `Arc<dyn T>`, `&dyn T`) to structs with auto-derived traits.
